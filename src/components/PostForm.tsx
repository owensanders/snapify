...
const PostForm = ({
  ...
  const [title, setTitle] = useState<string | undefined>(titleInitialValue);
  const [body, setBody] = useState<string | undefined>(bodyInitialValue);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isCreate) {
        ...
        response = await axios.put<CreatePostResponse>(
      ...
      if (axiosError.response && axiosError.response.status === 422) {
        ...
        console.error("Post create or update failed:", axiosError.message);
      }
    } finally {
      setLoading(false);
    }
